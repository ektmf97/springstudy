...
public class BlogServiceImpl implements BlogService {
  ...
  public void loadBlogList(HttpServletRequest request, Model model) {
    ...
    model.addAttribute("pagination", pageUtil.getPagination(request.getContextPath() + "/blog/list.do")); // 이 주소는 <1 2 3 4 5> 이 페이지네이션이 쓴다
  ...
  public void addBlog(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if(addResult == 1 ) {
        ...
        out.println("alert('블로그가 작성이 실패했습니다.');");
  ...
  public Map<String, Object> imageUpload(MultipartHttpServletRequest multipartRequest) {
    ...
    String filesystemName = myFileUtil.getFilesystemName(multipartFile.getOriginalFilename()); // 원래 파일이름 저장
    ...
    // HDD에 저장된 파일의 확인을 위한 mapping 값을 반환해준다. (servlet-context.xml 참고할 것)
    Map<String, Object> map = new HashMap<String, Object>();
