...
@Service
public class EmployeeListIServiceImpl  implements EmployeeListService {
	...
	public void getEmployeeListUsingPagination(HttpServletRequest request, Model model) {
		...
		// 파라미터 column이 전달되지 않은 경우 column=EMPLOYEE_ID로 처리한다.
		...
		switch(order) {
		case "ASC" : model.addAttribute("order","DESC"); break; // 현재 ASC 정렬이므로 다음 정렬은 DESC이라고 Jsp에게 알려준다.
