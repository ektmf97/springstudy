...
public class MvcController {
	...
	public String detail(HttpServletRequest request, Model model) {
		...
		// 이름은 null처리 안하고 나이는 null처리 함 
		Optional<String> opt1 = Optional.ofNullable(request.getParameter("name"));
		String name = opt1.orElse("홍길동"); // name 전달이 없으면 "홍길동"이 사용된다.
		...
		/*
		 	...
		 	2. Model2(Jsp/Servlet)에서는 HttpServletRequest request 객체를 사용해서 데이터를 전달하지만, 
	...
	/*
	    ...
	    public String detailGdu(@ModelAttribute (value="p") Person person) {  // Model에 저장할 속성명을 p로 해주세요!
										                                      // detail.jsp에서는 ${p.name}, ${p.age}와 같은 형식으로 사용할 수 있다.
		return "mvc/detail";
	}
