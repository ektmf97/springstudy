...
public class BoardServiceImpl implements BoardService {
	...
	@Override
	public BoardDTO detail1(int boardNo) { // 컨트롤러에서 받아온 10을
		return boardDAO.detail1(boardNo);  // 다오에 전달
	} 
	...
	public void detail3(Model model) {
		Map<String, Object> map = model.asMap(); // model을 map으로 바꿔줌
		HttpServletRequest request = (HttpServletRequest)map.get("result"); // 캐스팅 
		Optional<String> opt = Optional.ofNullable(request.getParameter("boardNo"));
		int boardNo = Integer.parseInt(opt.orElse("0"));
