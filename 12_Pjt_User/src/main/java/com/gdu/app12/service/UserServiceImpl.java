...
public class UserServiceImpl implements UserService {
  ...
  public void join(HttpServletRequest request, HttpServletResponse response) {
    ...
    // 이름 XSS 처리 자바스크립트 공격을 피하기 위해서 
    name = securityUtil.preventXSS(name); // 사용자가 입력한 이름을 가공해서 다시 바꿔주는 작업
    
    // 출생월일(월 + 일)
    birthdate = birthmonth + birthdate; // 0으로 시작 스트링처리 0101
    ...
    try {
      ...
      if(joinResult == 1) {
        ...
        out.println("location.href='"+ request.getContextPath()+"/index.do';"); // main페이지로 돌아감
      } else {
        out.println("alert('회원 가입에 실패했습니다.');");
        out.println("history.go(-2);"); // 2단계 전으로 가라 약관 페이지 
      }
      out.println("</script>");
      out.flush();
      out.close();
  ...
  public void autoLogin(HttpServletRequest request, HttpServletResponse response) {
    ...
    if(chkAutoLogin != null) {
      ...
      // session의 id를 가져온다. db로도 가고 쿠키로도 가야한다
      HttpSession session = request.getSession();
      ...
      Cookie cookie = new Cookie("autoLoginId", sessionId); // 자동로그인할 때 사용하는 id = sessionId
  ...
  public void leave(HttpServletRequest request, HttpServletResponse response) {
    // 탈퇴할 회원외 ID는 session에 loginId 속성으로 저장되어 있다.
    // 파라미터 안 넘기고 꺼내쓴다
    ...
    try {
      ...
      if(insertResult == 1 && deleteResult == 1) {
        ...
        out.println("history.go();"); // 2단계 전으로 가라 약관 페이지 
      }
      out.println("</script>");
      out.flush();
      out.close();
