...
public class SecondServiceImpl implements ISecondService {
	...
	public ResponseEntity<BmiVO> execute1(HttpServletRequest request) {
		...
		try {
			...
			return new ResponseEntity<BmiVO>(new BmiVO(weight, height, bmi, obesity), HttpStatus.OK);  // 객체 , 응답코드 ok는 (200)
			
		} catch (Exception e) {
			
			BmiVO bmiVO = null;
			return new ResponseEntity<BmiVO>(bmiVO, HttpStatus.INTERNAL_SERVER_ERROR); // 서버 에러(500을 뜻 함)
			// HttpStatusrk 500dlamfh $.ajax의 error에서 처리됨
		}
	}
	...
	public BmiVO execute1(HttpServletRequest request, HttpServletResponse response) {
			...
			try {
				...
				out.println("몸무게와 키 입력을 확인하세요."); // $.ajax의 error로 넘기는 예외
				out.flush();
				out.close();
			} catch (Exception e2) {
				e2.printStackTrace();
