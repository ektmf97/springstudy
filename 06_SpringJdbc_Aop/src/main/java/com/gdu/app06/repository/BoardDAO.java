...
public class BoardDAO {
	...
	public List<BoardDTO> selectBoardList(){
		...
		List<BoardDTO> list = jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(BoardDTO.class)); // 2번째 사용
		return list;
	}
	...
	public BoardDTO selectBoardByNo(final int BOARD_NO) { // 매개변수의 변조를 막기 위해서 final 처리한다. (예전에 매개변수 해킹 시도가 있었다.)
		...
		BoardDTO board = jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(BoardDTO.class), BOARD_NO); // 6번째 사용
		return board;
	}
	
	// 3. 삽입
	public int insertResult(final BoardDTO BOARD) {
		...
		int result = jdbcTemplate.update(sql, new PreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps) throws SQLException {
				ps.setString(1, BOARD.getTitle());  // ?에 집어넣어주세요!
	...
	// 4. 수정
	public int updatebBoard(final BoardDTO BOARD) {
