...
public class DBConfig {
	...
	@Bean()
	public DriverManagerDataSource dataSource() { // 메소드명 : dataSource() 
		DriverManagerDataSource dataSource = new DriverManagerDataSource();
		dataSource.setDriverClassName("oracle.jdbc.OracleDriver");  // 클래스 이름은 .으로 연결
		dataSource.setUrl("jdbc:oracle:thin:@localhost:1521:xe");   // 경로는 : 으로 연결
	...
	@Bean
	public TransactionManager transactrionManager() {
	...
	public TransactionInterceptor transactionInterceptor() {
		...
		ruleBasedTransactionAttribute.setRollbackRules(Collections.singletonList(new RollbackRuleAttribute(Exception.class))); // 언제 취소를 할 거냐
		
		MatchAlwaysTransactionAttributeSource source = new  MatchAlwaysTransactionAttributeSource();
		source.setTransactionAttribute(ruleBasedTransactionAttribute);
		
		return new TransactionInterceptor(transactrionManager(), source);
	}
